...
You'd like to analyze and visualize your data. Your workflow will be the most flexible, powerful, and reproducible if it involves *programming*. This type of programming is now called *data science*.
...
### R Markdown
This HTML is written in [R Markdown](https://rmarkdown.rstudio.com/), which weaves together text, figures, and code. You can "Knit" R Markdown documents into HTML, PDF, Microsoft Word, or other formats by pressing the "Knit" icon in R Studio. (This requires you to install the `knitr` package, which can be done with `install.packages("knitr", repos = "https://cloud.r-project.org")`, and the packages used below, `ezlimma`, `ezlimmaplot`, and `ggplot2`.)
...
`which()` identifies the values that are `TRUE`
...
`browseVignettes()` will show you the vignette.
...
`toptab` is ordered by overall significance of the 3 tests. We round the number of significant digits in `toptab` to 3 with `signif`. When we add annotation, we make sure the rows of `toptab` match the rows of `annot` using `annot[rownames(toptab),]`. We write this to `diff_gene_stats.csv`.
