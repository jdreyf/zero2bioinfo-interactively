...
![RStudio](./images/rstudio-workbench.png)
...
rownames(m) <- c("FB", "CB") #Affecting rownames to m 
m
```
...
```{r exp2, exercise=TRUE, exercise.eval = TRUE}
pv <- rep(NA, times=100)
pv
```
```{r}
...
```
```{r ezlimma01, exercise=TRUE, exercise.eval = TRUE}
...
```{r ezlimma02, exercise=TRUE, exercise.eval = TRUE}
